...
--
-- Slice C1.6a (migration 0031); quotas made POLICY-SOURCED by 0032 (upload contract unification). The atomic
-- evidence-carrying submit door. Executor uploads both PDFs direct-to-storage (RLS-gated by 0030), then this
-- creates the two documents rows + the claim in ONE transaction. Order upload->RPC: REQUIRES each object to
-- exist (no orphan rows) and reads size/mime FROM storage.objects (never client-trusted).
--
-- MULTI-SOURCE AGREEMENT: reject unless (a) p_estate, (b) the estate in EACH storage_path (strict regex), (c)
-- the claim's estate_id (=p_estate), (d) is_estate_executor all agree -> evidence_path_mismatch closes the
-- cross-estate smuggling vector.
--
-- 0032: the per-file / aggregate / file-count / MIME gates now read public.upload_policy (the SAME numbers
-- get_upload_policy tells the client) instead of hardcoded constants -> no drift between told and enforced.
-- Fail-closed if the singleton is absent (upload_policy_missing). EXECUTE authenticated, gate inside.
