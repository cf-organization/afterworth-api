/**
 * POST /api/vault/grants/revoke
 *
 * Owner-only: revoke an access grant by id. Thin wrapper over the SECURITY DEFINER rpc
 * `revoke_document_grant` (owner-gated, atomic status/revoked_at/revoked_by, audited).
 * Forwards the caller's JWT; passes the RPC's status + message through.
 *
 * Request:  Authorization: Bearer <JWT>; body { grantId }
 * Response (200): { grant: GrantWire }  (the revoked row)
 * Errors (RPC SQLSTATE -> HTTP, message passed through):
 *   42501 -> 403 (not owner)   P0001 -> 400 (grant_not_found)   else -> 502
...
function parseBody(raw: unknown): RevokeGrantBody | null {
...
export async function POST(req: Request): Promise<Response> {
  ...
  const rateLimitResponse = await enforce(req, "vault_grants_revoke");
  ...
  if (error) {
    ...
    if (mapped) {
      console.error("revoke_document_grant raised:", error.code, error.message);
      return mapped;
    }
  ...
  if (!Array.isArray(data) || data.length === 0) {
    console.error("revoke_document_grant returned unexpected shape:", data);
